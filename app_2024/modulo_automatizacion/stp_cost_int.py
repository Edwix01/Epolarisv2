        ...
        for host, config in datos_yaml[grupo]['hosts'].items():
            ...
            try: 
                if marca in ['3COM', 'HPV1910']:
                    ...
                    config_stp_cost.configurar_stpCost_3com(ip, user, password, interfaz, modo, costo, vlan, instance, save_config=False)
                    ...
                    if connection:
                        if marca == 'CISCO':
                            config_stp_cost.configurar_stpCost_cisco(connection, interfaz, costo, save_config=False)

                        elif marca == 'HPA5120':
                            config_stp_cost.configurar_stpCost_hp(connection, interfaz, modo, costo, vlan, instance, save_config=False)
