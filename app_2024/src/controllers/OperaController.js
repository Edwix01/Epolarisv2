...
function stp(req, res) {
  ...
  function cargarDatos() {
    try {
        const filePath = path.join(__dirname, '../../Epops/balanceo/balanceo_web.yaml');
  ...
  function procesarBalanceo(datos) {
    ...
    const filePath = path.join(__dirname, '..', '..', 'Epops', 'balanceo', 'balanceo_datos.yaml');
    const conexionesDisp = yaml.load(fs.readFileSync(filePath, 'utf8'));
  
    // Leer el archivo YAML de los dispositivos
    const devicesFilePath = path.join(__dirname, '..', '..', 'Epops', 'inventarios', 'dispositivos.yaml');
  ...
  function run_script(req, res) {
    ...
    const scriptPath = path.join(__dirname, '..', 'inicializacion','epolaris_install.sh');
